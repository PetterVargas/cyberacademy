...
Para no tener que ejecutar `hyperframes add <efecto>` (con llamada a red) cada
vez que se necesita un efecto, este proyecto tiene **los 372 items del
registro de HyperFrames** (154 blocks + 218 components, 370 instalados con
éxito) descargados como archivos `.html` locales en
...
- **Blocks** (`compositions/*.html`) — composiciones completas y listas para
  ...
  `data-chart`), UI mockeada de apps, transiciones con shaders, efectos de
  código (`code-diff`, `code-morph`, `code-particle-assemble`), mapas, etc.
- **Components** (`compositions/components/*.html`) — piezas reutilizables
  ...
  animados, transiciones, tipografía cinética, gráficos de datos, marcos de
  dispositivo, etc.
...
**Cómo usar un efecto del catálogo:**

1. Búscalo por nombre o tag: `pnpm exec hyperframes catalog --tag transition`
   o `pnpm exec hyperframes catalog --query "contador de dinero"`.
2. Ábrelo directo desde `catalogo-completo/compositions/` (o
   `.../components/`) para ver el HTML/CSS/JS de referencia.
3. Cópialo/adáptalo dentro del `index.html` propio, o instálalo con
   `pnpm exec hyperframes add <nombre> --dir <tu-proyecto>` (usa red la
   primera vez si el registro no está cacheado — `catalogo-completo/` ya
   tiene el contenido para copiar sin red).

**Actualizar el catálogo local:** `./scripts/update-catalog.sh` reinstala
(con `--force`) todos los items actuales; para uno solo,
`pnpm exec hyperframes add <nombre> --dir catalogo-completo --force`. Ver
detalle completo en el `README.md`.
