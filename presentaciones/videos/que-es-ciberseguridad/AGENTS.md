...
> **This project always renders at 60fps.** The composition's `<html data-fps="60">` is only a hint — `hyperframes render` does **not** read it, and defaults to 30fps when `-f`/`--fps` is omitted. `npm run render` already bakes in `-f 60`, so the default script is safe. If you render via the raw CLI instead (`npx hyperframes render .` or the `hyperframes-cli` skill's render step), always pass `-f 60` explicitly. Verify any render with `ffprobe -v error -select_streams v:0 -show_entries stream=r_frame_rate <file>` before handing it off — it should read `60/1`.
